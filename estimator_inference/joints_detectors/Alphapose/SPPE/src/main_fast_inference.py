...
class InferenNet(nn.Module):
    def __init__(self, kernel_size, dataset):
        ...
        print('Loading pose model from {}'.format('PoseTriplet/estimator_inference/joints_detectors/Alphapose/models/sppe/duc_se.pth'))
        sys.stdout.flush()
        model.load_state_dict(torch.load('PoseTriplet/estimator_inference/joints_detectors/Alphapose/models/sppe/duc_se.pth'))
        model.eval()
        self.pyranet = model
